...
def identify():
	...
	for line in file_in:
		...
		if (line[0].isalpha() or line[0]=='-' or line[0]=='#'): # New bridging object
			...
			graph_file = open("./graphs/" +graph_name+ ".py", 'w')
			graph_file.write("import networkx as nx\nfrom networkx_viewer import Viewer\n")
			graph_file.write("G = nx.DiGraph()\n")
			#graph = nx.Digraph(comment='graph_name')
					...
					if ('#'+key+'#' in read) or (' '+key+';' in read):	
						for write in writes:
							...
							if write_fields[3] == "*;":
								...
								# ADD EDGE ONLY IF NOT ALREADY EXISITNG
								#if test == 1 :		
								
								graph_file.write("G.add_edge('"+read_fields[1]+"','"+write_fields[1]+"')\n")

								graph_file.write("G.edge['"+read_fields[1]+"']['"+write_fields[1]+"']['read-write'] = '* >"+key+"'\n")
								...
								for value in ops_map.get(key):
									if  ('#'+value+'#' in write) or (' '+value+';' in write) :								
										...
										if read_fields[1]+" "+write_fields[1] in labels_dic:
											labels_dic[read_fields[1]+" "+write_fields[1]] = [labels_dic.get(read_fields[1]+" "+write_fields[1])[0]+"["+value+" "+key+"]"]
										else:
											labels_dic[read_fields[1]+" "+write_fields[1]] = ["["+value+" "+key+"]"]
										graph_file.write("G.edge['"+read_fields[1]+"']['"+write_fields[1]+"']['write-read'] = '"+labels_dic.get(read_fields[1]+" "+write_fields[1])[0]+"'\n")

										if ((value == 'write') and (key == 'read')):
											graph_file.write("G.edge['"+read_fields[1]+"']['"+write_fields[1]+"']['fill'] = 'red'\n") # if one is write read then the edge becomens red
