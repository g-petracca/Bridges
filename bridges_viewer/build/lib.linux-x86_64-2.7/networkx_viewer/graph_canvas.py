...
class GraphCanvas(tk.Canvas):
    ...
    def __init__(self, graph, **kwargs):
        ...
        #  displayed.
	#self.dispG = nx.MultiGraph() # gxp18
        self.dispG = nx.MultiDiGraph() # gxp18 - Solved Problem with Double linked nodes
	...
	#print "edge \n"
        ...
        if len(edges) == 0: # gxp 18 it was == 1
            m = 0
        else:
            m = 15 # this is how much an edges curves originally 15
	...
	### GXP 18 Need to do something here ###
	### Actually need to make sure node is in the edge set

        # Redraw any edges
        #from_node = self._drag_data['item'] # moved node
	#to_node2 = from_node #gxp18
        
	#from_xy = self._node_center(from_node)	# the new edge start from the moved 
	#to_xy2 = self._node_center(to_node2) #gxp18
	...
	#Redraw all the edges
        ...
        for to_node, from_node, data in self.dispG.edges_iter(data=True):
	    i = i + 1
            from_xy = self._node_center(from_node) # position of from node
            to_xy = self._node_center(to_node) # position of to node
            if data['dispG_frm'] != from_node:
                # Flip!
                a = from_xy[:]
                from_xy = to_xy[:]
                to_xy = a[:]
            spline_xy = self._spline_center(*from_xy+to_xy+(data['m'],)) # draw line

            data['token'].coords((from_xy+spline_xy+to_xy)) # add coordinets
	    #data['token'].coords((to_xy+spline_xy+from_xy))

	#print ("edges visited = " + str(i))
	...
	#print self.dispG.edges()
        ...
        #    if edge['dispG_frm'] != from_node:
                # Flip!  # Moves edge if point to
        #    	spline_xy = self._spline_center(*to_xy+from_xy+(edge['m'],))
        #    	edge['token'].coords((to_xy+spline_xy+from_xy))
	#    else: # Moves edge if exiting from
        #        spline_xy = self._spline_center(*from_xy+to_xy+(edge['m'],))
        #        edge['token'].coords((from_xy+spline_xy+to_xy))
	
	# gxp18
	#for _, from_node2, edge in self.dispG.edges_iter(to_node2, data=True):
        #    from_xy2 = self._node_center(from_node2)
        #    if edge['dispG_frm'] != to_node2:
                # Flip!  # Moves edge if point to
        #    	spline_xy = self._spline_center(*from_xy2+to_xy2+(edge['m'],))
        #    	edge['token'].coords((from_xy2+spline_xy+to_xy2))
        #    else: # Moves edge if exiting from
        #        spline_xy = self._spline_center(*to_xy2+from_xy2+(edge['m'],))
        #        edge['token'].coords((to_xy2+spline_xy+from_xy2))
